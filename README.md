...
解决: 在之前的实现里, 调用reject后, 仅仅只是去rejectionQueue里执行了相应的rejectionHandler. 其实除此之外, 还要去resolutionQueue里reject("拒绝")掉所有在then里return的promises. 这样error就会bubble up到最后的catch.
